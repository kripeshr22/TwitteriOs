...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    static let client = TwitterAPICaller(baseURL: URL(string: "https://api.twitter.com"), consumerKey: "mO0o95JudGvg7AxFLfcsfnoUg", consumerSecret: "xtqs5lsN1uYsZhDjhcmoGWvMO6N0ifmaTXA8oHBTRtjAOTK8FV")
    var loginSuccess: (() -> ())?
    ...
    func postTweet(tweetString: String, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        ...
        let myParams = ["status": tweetString]
        TwitterAPICaller.client?.post(url, parameters: myParams, progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
